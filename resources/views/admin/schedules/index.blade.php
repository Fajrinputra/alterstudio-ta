    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="mt-1 font-display text-3xl font-semibold tracking-tight text-[#3F2B1B] md:text-5xl md:tracking-[-1px]">
    ...
    <div class="bg-[#FAF6F0] py-5 sm:py-8">
        <div class="max-w-7xl mx-auto space-y-5 px-0 sm:px-6 lg:px-8 sm:space-y-8">
            ...
            <div class="relative group">
                ...
                <div class="relative glass border border-[#EDE0D0] rounded-3xl p-4 shadow-xl backdrop-blur-2xl sm:p-6 lg:p-8">
                    <form method="GET" class="flex flex-wrap items-end gap-4 sm:gap-6">
                        <div class="flex-1 min-w-0 sm:min-w-[200px]">
                            ...
                            <select name="package_id" 
                                    class="w-full rounded-3xl border border-[#E1D3C5] bg-white/70 px-4 py-3 text-sm text-[#3F2B1B] backdrop-blur-md transition-all focus:border-[#D4A017] focus:ring-2 focus:ring-[#D4A017]/20 sm:px-5 sm:py-4 sm:text-base">
                        ...
                        <div class="flex w-full sm:w-auto">
                            <button class="flex h-12 w-full items-center justify-center gap-3 rounded-3xl bg-gradient-to-r from-[#D4A017] to-[#E07A5F] px-6 text-sm font-semibold text-white shadow-xl shadow-[#D4A017]/30 transition-all hover:-translate-y-0.5 hover:shadow-2xl active:scale-[0.98] sm:h-14 sm:w-auto sm:px-8 sm:text-base">
            ...
            <div class="space-y-5">
                @forelse($projects as $project)
                    ...
                    <div class="relative group">
                        <div class="absolute inset-0 bg-gradient-to-br from-[#D4A017]/5 via-[#E07A5F]/5 to-transparent rounded-2xl blur-2xl"></div>
                        <div class="relative bg-white/85 backdrop-blur-xl border border-[#EDE0D0] rounded-2xl p-4 shadow-lg hover:shadow-xl transition-all duration-300 sm:p-5">
                            
                            {{-- Header --}}
                            <div class="mb-4 flex flex-wrap items-start justify-between gap-4">
                                <div class="flex items-start gap-3">
                                    <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#D4A017]/10 to-[#E07A5F]/10 sm:h-11 sm:w-11">
                                        <span class="font-mono text-[#D4A017] font-semibold text-base">#{{ $project->booking_id }}</span>
                                    </div>
                                    <div>
                                        <h3 class="font-display text-lg text-[#3F2B1B] leading-tight sm:text-xl">{{ $project->booking->package->name ?? '-' }}</h3>
                                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[#7A5B3A] mt-1.5">
                                ...
                                <div class="flex w-full max-w-sm flex-col items-end gap-2 sm:w-auto">
                                    <span class="inline-flex items-center px-3 py-1.5 rounded-2xl text-xs font-medium {{ $statusBadge['color'] }}">
                                        {{ $statusBadge['label'] }}
                                    </span>
                                    @if($project->schedule)
                                        <span class="inline-flex items-center px-3 py-1.5 rounded-2xl text-xs font-medium bg-emerald-100 text-emerald-700 gap-2">
                                            <i class="fa-solid fa-check"></i> Terjadwal
                                        </span>
                                        @if($isPhotographer)
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-2xl text-xs font-medium bg-blue-100 text-blue-700 gap-2">
                                                <i class="fa-solid fa-camera"></i> Fotografer
                                            </span>
                                        @endif
                                        @if($isEditor)
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-2xl text-xs font-medium bg-orange-100 text-orange-700 gap-2">
                                                <i class="fa-solid fa-pen-ruler"></i> Editor
                                            </span>
                                        @endif
                                        @if(!(isset($readOnly) && $readOnly))
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-2xl text-xs font-medium {{ $scheduleManageBadge['color'] }}">
                            ...
                            {{-- Schedule Info Box --}}
                            <div class="grid md:grid-cols-2 gap-4 mb-4 p-4 bg-[#FAF6F0] rounded-2xl border border-[#EDE0D0]">
                                <div>
                                    <p class="text-[11px] tracking-widest text-[#8B7359] mb-1.5">JADWAL PEMESANAN</p>
                                    <p class="text-sm text-[#3F2B1B] font-medium flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-[11px] tracking-widest text-[#8B7359] mb-1.5">STATUS PEMBAYARAN</p>
                                    <p class="text-sm text-[#3F2B1B] font-medium">{{ $bookingStatus }}</p>
                                </div>
                            </div>
                            ...
                            @if(isset($readOnly) && $readOnly)
                                ...
                                <div class="grid md:grid-cols-2 gap-4 mb-4 p-4 bg-[#FAF6F0] rounded-2xl border border-[#EDE0D0]">
                                    <div>
                                        <p class="text-[11px] tracking-widest text-[#8B7359] mb-1.5">FOTOGRAFER</p>
                                        <p class="text-sm text-[#3F2B1B] font-medium flex items-center gap-2">
                                    ...
                                    <div>
                                        <p class="text-[11px] tracking-widest text-[#8B7359] mb-1.5">EDITOR</p>
                                        <p class="text-sm text-[#3F2B1B] font-medium flex items-center gap-2">
                                ...
                                @if($project->schedule)
                                    <div class="border-t border-[#EDE0D0] pt-4">
                                        ...
                                        @if($canContinueProduction && $isPhotographer)
                                            ...
                                            <div class="space-y-4">
                                                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-blue-50 text-blue-700 text-xs font-medium">
                                                    <i class="fa-solid fa-camera"></i>
                                                    Anda bertugas sebagai Fotografer
                                                </div>
                                                <h4 class="font-display text-lg text-[#3F2B1B] flex items-center gap-2">
                                                    <i class="fa-brands fa-google-drive text-[#D4A017]"></i>
                                                    Bagikan Link Drive Foto Mentah
                                                </h4>
                                                
                                                @if(!$project->raw_drive_url)
                                                    <p class="text-sm text-[#7A5B3A]">Upload foto mentah ke Google Drive, lalu tempel link folder di sini. Klien akan diberi tahu bahwa link berlaku selama 3 hari.</p>
                                                    <form method="POST" action="{{ route('projects.drive-assets.store', $project) }}" class="mt-3">
                                                        ...
                                                        <div class="flex flex-col gap-4 sm:flex-row">
                                                            <input type="url" name="raw_drive_url" required placeholder="https://drive.google.com/..."
                                                                   class="flex-1 rounded-2xl border border-[#E1D3C5] bg-white px-4 py-2.5 text-sm focus:border-[#D4A017] focus:ring-2 focus:ring-[#D4A017]/20">
                                                            <button class="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-[#D4A017] to-[#E07A5F] text-sm text-white font-semibold hover:shadow-xl transition-all">
                                        ...
                                        @if($canContinueProduction && $isEditor)
                                            ...
                                            <div class="space-y-4 mt-5">
                                                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-orange-50 text-orange-700 text-xs font-medium">
                                                ...
                                                @else
                                                    <p class="rounded-2xl border border-[#EDE0D0] bg-[#FAF6F0] p-4 text-sm text-[#7A5B3A]">
                                                        Menunggu klien mengirim kode foto dan deskripsi edit.
                                                    </p>
                                                @endif

                                                @if($project->hasEditRequest() && !$project->hasFinalDelivery())
                                                    <form method="POST" action="{{ route('projects.drive-assets.store', $project) }}" class="space-y-3 rounded-2xl border border-[#EDE0D0] bg-white p-4">
                                                        @csrf
                                                        <input type="hidden" name="type" value="FINAL">
                                                        <div>
                                                            <label class="block text-xs font-medium text-[#5C432C] mb-1.5">Link Drive Hasil Final</label>
                                                            <p class="mb-2 text-xs text-[#7A5B3A]">Klien akan diberi tahu bahwa link hasil final berlaku selama 3 hari.</p>
                                                            <input type="url" name="final_drive_url" value="{{ $project->raw_drive_url }}" required
                                                                   class="w-full rounded-2xl border border-[#E1D3C5] bg-[#FAF6F0] px-4 py-2.5 text-sm focus:border-[#D4A017] focus:ring-2 focus:ring-[#D4A017]/20">
                                                        </div>
                                                        <div>
                                                            <label class="block text-xs font-medium text-[#5C432C] mb-1.5">Pesan ke Klien</label>
                                                            <textarea name="final_message" rows="2"
                                                                      class="w-full rounded-2xl border border-[#E1D3C5] bg-[#FAF6F0] px-4 py-2.5 text-sm focus:border-[#D4A017] focus:ring-2 focus:ring-[#D4A017]/20"
                                                                      placeholder="Contoh: Hasil edit sudah kami upload di folder Drive."></textarea>
                                                        </div>
                                                        <button class="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-sm text-white font-semibold hover:shadow-xl transition-all">
                            ...
                            @else
                                ...
                                @if(!$project->schedule)
                                    <form method="POST" action="/projects/{{ $project->id }}/schedule" 
                                          class="grid items-end gap-4 lg:grid-cols-2 lg:gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(280px,0.9fr)]">
                                        @csrf
                                        <div>
                                            ...
                                            @if(count($disabledPhotographers))
                                                <p class="mt-2 text-xs text-[#8B7359]">
                                                    <i class="fa-solid fa-circle-info mr-1 text-[#D4A017]"></i>
                                                    Opsi nonaktif berarti sudah terjadwal.
                                                </p>
                                            @endif
                                        </div>
                                        <div>
                ...
                @empty
                    <div class="text-center py-12 bg-white/70 backdrop-blur-2xl border border-[#EDE0D0] rounded-3xl sm:py-20">
                        <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-[#F4EDE4] flex items-center justify-center sm:mb-6 sm:h-24 sm:w-24">
                            <i class="fa-solid fa-calendar-xmark text-3xl text-[#8B7359] sm:text-5xl"></i>
                        </div>
